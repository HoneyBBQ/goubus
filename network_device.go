...
// NetworkDeviceStatus retrieves the status of a specific network device.
func (u *Client) networkDeviceStatus(name string) (UbusNetworkDevice, error) {
	errLogin := u.LoginCheck()
	if errLogin != nil {
		return UbusNetworkDevice{}, errLogin
	}

	jsonStr := u.buildUbusCall("network.device", "status", map[string]string{"name": name})
